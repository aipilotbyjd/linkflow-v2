...
type WorkflowTemplate struct {
	...
	Nodes       models.JSON `json:"nodes"`
	Connections models.JSON `json:"connections"`
	...
	return []WorkflowTemplate{
		{
			...
			Nodes:       parseJSON(`[{"id":"webhook","type":"trigger.webhook","name":"Webhook Trigger","position":{"x":100,"y":200},"config":{"method":"POST","path":"/webhook"}},{"id":"slack","type":"integration.slack","name":"Send Slack Message","position":{"x":400,"y":200},"config":{"operation":"postMessage","channel":"#general","text":"{{$json.message}}"}}]`),
			Connections: parseJSON(`[{"source":"webhook","target":"slack"}]`),
			Settings:    parseJSON(`{}`),
		},
		{
			ID:          "scheduled-report",
			...
			Nodes:       parseJSON(`[{"id":"schedule","type":"trigger.schedule","name":"Daily Schedule","position":{"x":100,"y":200},"config":{"cron":"0 9 * * *","timezone":"UTC"}},{"id":"http","type":"action.http","name":"Fetch Data","position":{"x":300,"y":200},"config":{"method":"GET","url":"https://api.example.com/report"}},{"id":"email","type":"integration.email","name":"Send Report","position":{"x":500,"y":200},"config":{"to":"team@example.com","subject":"Daily Report","body":"{{$json}}"}}]`),
			Connections: parseJSON(`[{"source":"schedule","target":"http"},{"source":"http","target":"email"}]`),
			Settings:    parseJSON(`{}`),
		},
		{
			ID:          "github-pr-notify",
		...
		},
		{
			...
			Nodes:       parseJSON(`[{"id":"trigger","type":"trigger.manual","name":"Manual Trigger","position":{"x":100,"y":200},"config":{}},{"id":"openai","type":"integration.openai","name":"Generate Content","position":{"x":300,"y":200},"config":{"operation":"chatCompletion","model":"gpt-4","messages":[{"role":"user","content":"Write a blog post about: {{$json.topic}}"}]}},{"id":"notion","type":"integration.notion","name":"Create Page","position":{"x":500,"y":200},"config":{"operation":"createPage","parentId":"{{$json.notionParentId}}","title":"{{$json.topic}}","content":"{{$node.openai.json.choices[0].message.content}}"}}]`),
			Connections: parseJSON(`[{"source":"trigger","target":"openai"},{"source":"openai","target":"notion"}]`),
			Settings:    parseJSON(`{}`),
		},
		{
			ID:          "data-sync",
		...
		},
		{
			...
			Nodes:       parseJSON(`[{"id":"schedule","type":"trigger.schedule","name":"Daily Backup","position":{"x":100,"y":200},"config":{"cron":"0 2 * * *","timezone":"UTC"}},{"id":"fetch","type":"action.http","name":"Export Data","position":{"x":300,"y":200},"config":{"method":"GET","url":"{{$env.API_URL}}/export"}},{"id":"compress","type":"action.code","name":"Compress","position":{"x":500,"y":200},"config":{"code":"const zlib = require('zlib'); return { data: zlib.gzipSync(JSON.stringify(items[0].json)).toString('base64'), filename: 'backup-' + new Date().toISOString().split('T')[0] + '.json.gz' }"}},{"id":"notify","type":"integration.slack","name":"Notify","position":{"x":700,"y":200},"config":{"operation":"postMessage","channel":"#ops","text":"✅ Backup completed: {{$json.filename}}"}}]`),
			Connections: parseJSON(`[{"source":"schedule","target":"fetch"},{"source":"fetch","target":"compress"},{"source":"compress","target":"notify"}]`),
			Settings:    parseJSON(`{}`),
		},
		{
